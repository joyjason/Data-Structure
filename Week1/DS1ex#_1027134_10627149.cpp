...
int main(void){
	...
	while ( fun != 0 ) {
      ...
      printf( "Input a choise<0, 1, 2, 3>:" );
      std::cin >> fun ;
      if ( fun == 0 || fun == 1 || fun == 2 || fun == 3 ) {
        if ( fun == 1) mission1();
	    if ( fun == 2) mission2(); 
        if ( fun == 3) mission3();
        if ( fun == 0) return 0;
...
void mission3() { 
    ...
    gall = gfirst;
    sort();
    for( gall = gfirst ; gall != NULL ; gall = gall->next){
    	std::cout << "[" << dash << "] " ; 
...
void sort(){
  ...
  for( All *temp = gfirst ; temp->next != NULL ; temp = temp->next){
  	for( All *temp2 = temp; temp2->next != NULL ; temp2 = temp2->next ){
  		if( temp2->gnum[0] *100 + temp2->gnum[1] * 10 + temp->gnum[2] > temp2->next->gnum[0] *100 + temp2->next->gnum[1] * 10 + temp2->next->gnum[2] ) {
